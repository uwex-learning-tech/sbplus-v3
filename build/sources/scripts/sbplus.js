...
layout:null,splash:null,banner:null,tableOfContents:null,widget:null,button:null,menu:null,screenReader:null,presentationLoc:"",logo:"",
...
var h=$(e),b=h.find("storybook"),f=h.find("setup"),v=t.trimAndLower(b.attr("accent")),S=t.trimAndLower(b.attr("pageImgFormat")),w="svg",_=t.trimAndLower(b.attr("analytics")),y="",k=b.attr("xmlVersion"),P="",L=t.trimAndLower(f.attr("course")),x=t.noScript(f.find("title").text().trim()),C=t.noScript(f.find("subtitle").text().trim()),I=f.find("length").text().trim(),E=f.find("author"),B=t.getTextContent(f.find("generalInfo")),U=h.find("section"),z=b.attr("splashImgFormat"),q=f.attr("program");
...
t.xml={settings:{accent:v,imgType:S,splashImgType:w,analytics:_,mathjax:y,version:k},setup:{program:P,course:L,title:x,subtitle:C,author:E,authorPhoto:"",duration:I,generalInfo:B},sections:U},
// get/set the presenation storage id
t.presentationLoc=t.sanitize(t.getCourseDirectory()),""!=t.manifest.sbplus_hotjar_site_id){var T=Number(t.manifest.sbplus_hotjar_site_id);u=window,d=document,c="https://static.hotjar.com/c/hotjar-",p=".js?sv=",u.hj=u.hj||function(){(u.hj.q=u.hj.q||[]).push(arguments)},u._hjSettings={hjid:T,hjsv:6},m=d.getElementsByTagName("head")[0],(g=d.createElement("script")).async=1,g.src=c+u._hjSettings.hjid+p+u._hjSettings.hjsv,m.appendChild(g)}
// if analytics is on, get and set Google analtyics tracking
if("on"===t.xml.settings.analytics&&(a=window,s=document,i="script",r="//www.google-analytics.com/analytics.js",n="ga",a.GoogleAnalyticsObject=n,a.ga=a.ga||function(){(a.ga.q=a.ga.q||[]).push(arguments)},a.ga.l=1*new Date,o=s.createElement(i),l=s.getElementsByTagName(i)[0],o.async=1,o.src=r,l.parentNode.insertBefore(o,l),ga("create",t.manifest.sbplus_google_tracking_id,"auto"),ga("set",{appName:"SBPLUS",appVersion:t.xml.settings.version})),E.length){
// set author name and path to the profile to respective variable
var A=t.sanitize(E.attr("name").trim()),M=t.manifest.sbplus_author_directory+A+".json";t.xml.setup.author=E.attr("name").trim(),
// if author data in XML is empty
t.isEmpty(E.text())&&!t.isEmpty(A)?
...
s.hasStorageItem("sbplus-"+s.presentationLoc)?
...
e.selectPage(e.getStorageItem("sbplus-"+e.presentationLoc))}),e.presentationStarted=!0,e.sendToGA("PresentationResumeBtn","click",e.getCourseDirectory(),0,0))},
/**
...
i.src=s.attr("src").trim(),
// if there is a note tag, set notes
s.find("note").length&&(i.notes=this.getTextContent(s.find("note"))),i.widget=s.find("widget"),i.frames=s.find("frame"),i.imageFormat=this.xml.settings.imgType,i.transition=s[0].hasAttribute("transition")?s.attr("transition").trim():"",
...
// volumne
var e=$("#sbplus_va_volume").val(),t=!1;e<0||100<e||a.isEmpty(e)?(t=!0,e=100*Number(a.getStorageItem("sbplus-volume"))):(a.setStorageItem("sbplus-volume",e/100),a.setStorageItem("sbplus-"+a.presentationLoc+"-volume-temp",e/100,!0)),t?$("#volume_label").after('<p class="error">Value must be between 0 and 100.</p>'):$("#volume_label").next(".error").remove(),
// playback rate
a.setStorageItem("sbplus-playbackrate",$("#sbplus_va_playbackrate option:selected").val()),a.setStorageItem("sbplus-"+a.presentationLoc+"-playbackrate-temp",$("#sbplus_va_playbackrate option:selected").val(),!0),
// show msg
$(a.menu.menuSavingMsg).html("Settings saved!"),setTimeout(function(){$(a.menu.menuSavingMsg).fadeOut("slow",function(){$(this).empty()})},1500)}))},syncSettings:function(){var e=this;if("1"===e.getStorageItem("sbplus-"+e.presentationLoc+"-settings-loaded",!0)){
// widget
